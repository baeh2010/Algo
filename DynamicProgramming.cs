...
namespace InterviewPractise_VSCODE
{
    public class DynamicProgramming{
        ...
        /*
        warm up. number of ways to climb N stairs 
         you can only climb 1 or 2 stairs at a time 
        1- return the number of ways 
        2- return the ways
...
// top down approach 
    public static Dictionary<int,int>memo = new Dictionary<int,int>();
     public static int ClimbStairsTopDown(int n){
        ...
        int result= ClimbStairsTopDown(n-1)+ClimbStairsTopDown(n-2);
        memo.Add(n,result);
        return result;
     }
    ...
    // we can move 1 or 2 or 3 stairs at a time 
    ...
    public static int TripleSteps(int n){
     ...
     if(memo.ContainsKey(n)){
      ...
      int result= TripleSteps(n-1)+TripleSteps(n-2)+TripleSteps(n-3);
      memo.Add(n,result);
      return result;
    }

    public static int TripleStepsBU(int n){
        int[]dp = new int[n+1];
        dp[0]=1;
        dp[1]=1;
        dp[2]=dp[1]+dp[0];
    ...
    // 8.2 robot in a grid find the leetcode version and slove it 

 
   // 8.3  recursion. return all subsets of a set

   public static List<int[]> AllSubSets(int[]set){
      List<int[]> result = new List<int[]>();
      all_subsets(0,set,result,new List<int>());
      return result;
   }
   private static void all_subsets(int i,int[] set,List<int[]>result,List<int>slate){
         if(i==set.Length){
           result.Add(slate.ToArray());
           return;
         }
         slate.Add(set[i]);
         all_subsets(i+1,set,result,slate);
         slate.RemoveAt(slate.Count-1);
         all_subsets(i+1,set,result,slate);
   }


   // another recursion problem print all valid paranthesis of length n
   // I have option for open paranthesis and close paranthasis
   public static List<string> ValidParanthesis(int n){
      List<string>result=new List<string>();
      valid_paranthesis(n,n,"",result);
      return result;
   }
   private static void valid_paranthesis(int open,int close,string slate,List<string>result){
       if(open<0||close<0)
       return;
       ...
       valid_paranthesis(open-1,close,slate+" (",result);
       if(open<close)
       valid_paranthesis(open,close-1,slate+" )",result);

   }
    /*
    number of was to make change given an infinit number of each coin
  
    */

// first recursive solution
// public static int result=0;
    public static int MakingChange(int n,int[]coins,List<int>slate){
       if(n<0)
       return 0;
       if(n==0){
       ...
       foreach(int coin in coins){
           if(n>=coin){
               slate.Add(coin);
        result+=MakingChange(n-coin,coins,slate);
               slate.RemoveAt(slate.Count-1);
           }
       }
       return result;
